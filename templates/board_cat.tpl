{strip}
	<div class="indent">
		...
		{elseif $gBitSystem->isPackageActive('pigeonholes') and count($ns) > 1}
			<a href="javascript:BitBase.toggle('bcatnone','block',true);"><h{$heading|default:2}>Uncategorized</h{$heading|default:2}></a>
		{/if}
		<div id="bcat{$child.data.content_id|default:none}">
			...
			{if !empty($child.sub_count) and count($child.children)>0}
