						...
						{foreach item=mapping from=$board.map}
							<tr class="{cycle values="odd,even"}">
								...
								<td class="actionicon">
									...
									<a title="Remove from board" href="{$smarty.const.BOARDS_PKG_URL}assign.php?remove[{$board.board_id}][{$mapping.t_content_id}]=1#board{$smarty.foreach.board_loop.iteration}">{biticon ipackage="icons" iname="edit-delete" ipackage="icons" iexplain="Remove from board"}</a>
								</td>
							</tr>
						{/foreach}
					</table>
